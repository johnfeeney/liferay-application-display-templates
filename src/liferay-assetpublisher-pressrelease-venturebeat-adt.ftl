<style>
 ...
 #river .article-wrapper .article-header h2, .h2 {
     font-size: 30px;
 }
 #river .article-wrapper .article-header h1, h2, h3, h4, h5, h6, .h1, .h2, .h3, .h4, .h5, .h6 {
     font-family: inherit;
 ...
 <div id="river" class="story-river" role="main">
 	<#list entries as curEntry>
        <#assign 
            ...
            document = saxReaderUtil.read(blogEntry.getContent())
            rootElement = document.getRootElement()
        />

 	    <#list rootElement.elements() as curElement>
 	        <#if curElement.attributeValue("name") == "image">
 	            <#assign json = curElement.element("dynamic-content").getText()/>
 	        </#if>
            
            <#if json?has_content>
                <#assign m = json?eval>
                <#assign imageSrc = themeDisplay.getPortalURL() + "/c/document_library/get_file?uuid=" + m.uuid + "&groupId=" + m.groupId/>
			<#else>
				<#assign imageSrc = themeDisplay.getPathThemeImages() + "/template/press-release-1280x500.jpg" />
            </#if>
 	    </#list>
